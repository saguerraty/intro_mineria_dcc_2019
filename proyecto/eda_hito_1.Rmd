...
enero <- ggplot(jan, aes(año, O3))+geom_point(mapping=aes(anno, O3))

ozono <- ggplot(with(monthly_data,monthly_data[mes < 4,]),aes(x=anno, y=O3)) + 
    geom_bar(stat="identity") + theme_minimal() +
    ggtitle("Tendencia de Ozono en Verano (Ene-Mar)") +
    xlab("Años") + ylab("O3 (ppb)")

lluvia <- ggplot(with(monthly_data,monthly_data[mes < 4,]),aes(x=anno, y=Precipitaciones)) + 
    geom_bar(stat="identity") + theme_minimal() +
    ggtitle("Tendencia de Precipitaciones en Verano (Ene-Mar)") +
    xlab("Años") + ylab("Precipitaciones (mm)")

grid.arrange(enero, ozono, lluvia, nrow=3)
```
